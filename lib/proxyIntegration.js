...
function process(proxyIntegrationConfig, request) {

    const event = request.proxyRequest
    const context = request.lambdaContext
    const callback = (err, resp) => {
      if(err) return request.lambdaContext.fail(err)
      return request.lambdaContext.succeed(resp)
    }

    if (proxyIntegrationConfig.debug) {
        console.log("Lambda proxyIntegrationConfig: ", proxyIntegrationConfig)
        console.log("Lambda request: ", request)
        console.log("Lambda event: ", event)
        console.log("Lambda context: ", context)
        console.log("Lambda callback: ", callback)
    }
    ...
    try {
        ...
        return Promise.resolve(actionConfig.action(event, context, callback))
            .then(body => {
                return {statusCode: 200, headers: headers, body: body}
            })
            .catch(err => convertError(err, errorMapping, headers))
    } catch (error) {
